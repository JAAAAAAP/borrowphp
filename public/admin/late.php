<div class="flex flex-col w-11/12 h-auto overflow-x-auto">
    ...
    $i = 1;
    $group_sql = "SELECT DISTINCT group_id FROM oder_product WHERE status IN ('เลยกำหนด') ORDER BY $select $sort";
                        ...
                        <table class="table text-center h-44">
                                ...
                                <tr class="text-black text-base text-nowrap ">
                                    ...
                                    <th class="">
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                <li><a href="?pt=late<?= isset($_GET['status']) ? '&&status=' . $_GET['status'] : '' ?>&&date=new">ใหม่ที่สุด</a></li>
                                                <li><a href="?pt=late<?= isset($_GET['status']) ? '&&status=' . $_GET['status'] : '' ?>&&date=old">เก่าที่สุด</a></li>
                                            </ul>
                                        </div>
                                    </th>
                                    ...
                                    <th class="">
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                <li><a href="?pt=late&&status=all">ทั้งหมด</a></li>
                                                <li><a href="?pt=late&&status=borrowing">กำลังยืม</a></li>
                                                <li><a href="?pt=late&&status=late">เลยกำหนด</a></li>
                                                <li><a href="?pt=late&&status=returned">คืนแล้ว</a></li>
                                            </ul>
                                        </div>
                                    </th>
                                </tr>
                            ...
                            <tbody>
                                ...
                                $product_sql = "SELECT o.*,u.user,p.name,p.amount AS products_amount,p.sn_products
                                            ...
                                            WHERE o.group_id = :group_id AND status IN ('เลยกำหนด') $approve
                                            ";
                                ...
                                ?>
                                        <tr class="text-base text-nowrap md:text-xl">
                                            ...
                                            <td>
                                                <div class="dropdown dropdown-end">
                                                    <div tabindex="0" role="button" class="btn text-white btn-<?= $product['status'] === "กำลังยืม" ? "info" : ($product['status'] === "เลยกำหนด" ? "error" : ($product['status'] === "คืนแล้ว" ? "success" : "")); ?>"><?= $product['status'] ?></div>
                                                    <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                        <?php if ($product['status'] === "กำลังยืม") { ?>
                                                            <li class=""><a href="\jaa\borrowphp\controller\order_approve.php?approve=return&&o=<?= $product['o_id'] ?>&&p=<?= $product['p_id'] ?>&&amount=<?= $product['amount'] ?>">คืนแล้ว</a></li>
                                                        <?php } elseif ($product['status'] === 'เลยกำหนด') { ?>
                                                            <li class=""><a href="\jaa\borrowphp\controller\order_approve.php?approve=return&&o=<?= $product['o_id'] ?>&&p=<?= $product['p_id'] ?>&&amount=<?= $product['amount'] ?>">คืนแล้ว</a></li>
                                                            <li class=""><a href="\jaa\borrowphp\controller\order_approve.php?approve=wait&&o=<?= $product['o_id'] ?>&&p=<?= $product['p_id'] ?>&&amount=<?= $product['amount'] ?>">กำลังยืม</a></li>
                                                        <?php } elseif ($product['status'] === 'คืนแล้ว') { ?>
                                                        <?php } ?>
                                                    </ul>
                                                </div>
                                            </td>
