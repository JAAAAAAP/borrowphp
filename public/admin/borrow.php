<div class="flex flex-col w-11/12 h-auto overflow-x-auto">
    ...
    $sort = "ASC";
    $approve = "";
    if (isset($_GET['pt']) && $_GET['pt'] === 'borrow') {
        if (isset($_GET['date'])) {
            if ($_GET['date'] === 'new') {
                $select = "dates_now";
    ...
    $i = 1;
    $group_sql = "SELECT DISTINCT group_id FROM oder_product WHERE status IN ('กำลังยืม', 'คืนแล้ว','เลยกำหนด') ORDER BY $select $sort";
                        ...
                        <table class="table text-center h-44">
                                ...
                                <tr class="text-black text-base text-nowrap ">
                                    ...
                                    <th class="">
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                <li><a href="?pt=borrow<?= isset($_GET['status']) ? '&&status=' . $_GET['status'] : '' ?>&&date=new">ใหม่ที่สุด</a></li>
                                                <li><a href="?pt=borrow<?= isset($_GET['status']) ? '&&status=' . $_GET['status'] : '' ?>&&date=old">เก่าที่สุด</a></li>
                                            </ul>
                                        </div>
                                    </th>
                                    ...
                                    <th class="">
                                        <div class="dropdown dropdown-end">
                                            ...
                                            <ul tabindex="0" class="dropdown-content z-[1] menu p-2 shadow bg-base-100 rounded-box w-52">
                                                <li><a href="?pt=borrow&&status=all">ทั้งหมด</a></li>
                                                <li><a href="?pt=borrow&&status=borrowing">กำลังยืม</a></li>
                                                <li><a href="?pt=borrow&&status=late">เลยกำหนด</a></li>
                                                <li><a href="?pt=borrow&&status=returned">คืนแล้ว</a></li>
                                            </ul>
                                        </div>
                                    </th>
                                </tr>
                            ...
                            <tbody>
                                ...
                                $product_sql = "SELECT o.*,u.user,p.name,p.amount AS products_amount,p.sn_products
                                            ...
                                            WHERE o.group_id = :group_id AND status IN ('กำลังยืม', 'คืนแล้ว','เลยกำหนด') $approve
                                            ";
